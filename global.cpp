...
int _initGlobalInfo(globalInfo_t* g, int argc, char* argv[])
{
    int ret = 0;
    ...
    // after first init common data
    //ret = pthread_mutex_init(g->mutex, NULL);
#if (1 == FLAG_USING_OPENCV)
    g->quality[0] = CV_IMWRITE_PNG_COMPRESSION;
    g->quality[1] = 0;  //0~100
    g->quality[2] = 0;
    g->imgYuv.create(g->height, g->width, CV_16UC1);    //CV_8UC2
    g->imgRgb.create(g->height, g->width, CV_8UC3);
#endif

    return ret;
}

int _exitGlobalInfo(globalInfo_t* g)
{
    int ret = 0;
...
#if (1 == FLAG_USING_OPENCV)
            memcpy(g->imgYuv.data, g->vbufp[buffer.index], g->frameSize);
#else
            DBG_PRINT(" g->frameSize:%d\n.", g->frameSize);
            if(g->argc > 1)
            {
                FILE* fp = fopen(g->argv[1], "r");
                if(fp)
                {
                    ret = fread(g->frameBuf, g->frameSize, 1, fp);
                    fclose(fp);
                }else
                {
                    ret = -EIO;
                    ERR_PRINT("Fread a frame(%s) failed.", g->argv[1]);
                }
            }else
            {
...
int _saveImgFrame(unsigned char* dst, unsigned char* src, int size)
{
	...
	//rm old file
    strcat((char*)cmd, (char*)dst);
    DBG_PRINT("%s(old file)", cmd);

    system((char*)cmd);

	DBG_PRINT();
    int fd = open((char*)dst, O_WRONLY|O_CREAT, 00700);//保存图像数据
    if(fd >= 0)
	{
...
int demoForSaveOneFrameWithoutOpenCv(int argc, char* argv[])
{
    ...
    _initGlobalInfo(&gInfo, argc, argv);
    ...
    else
    {
        ...
        {
            ret = _saveImgFrame((unsigned char*)IMG_SAVE_PATH, gInfo.frameBuf, gInfo.frameSize);
...
void _yuv2Rgb_yuv422(unsigned char *pYUV, unsigned char *pRGB, int pixNum)
{
   unsigned char y[2]={}, u = 0, v = 0;
   int r=0, g=0, b=0;
   ...
   while(pixNum-- > 0)
   {
       ...
       for(int i=0; i<2; i++)
       {
           r = 1.164*(y[i]-16) + 1.596*(v-128);
           g = 1.164*(y[i]-16) - 0.813*(v-128) - 0.392*(u-128);
           b = 1.164*(y[i]-16) + 2.017*(u-128);
